...
import { searchGoogleResults } from "@/lib/search/googleSearch";
...
async function getSearchFallbackArticles(seasonLabel, articleLimit) {
  try {
    const results = await searchGoogleResults(
      `Premier League ${seasonLabel || ""} latest news`,
      {
        limit: articleLimit,
      },
    );
    ...
    return results.map((result) => ({
      title: result.title,
      url: normalizeArticleUrl(result.url),
      source: result.source || "Google Search",
      publishedAt: null,
      summary: stripHtml(result.description).slice(0, 220),
      image: "",
    }));
  } catch {
    return [];
...
export async function getFootballNewsFeed({
  ...
  const searchFallbackArticles = await getSearchFallbackArticles(
    seasonLabel,
    Math.max(4, Math.min(articleLimit, 8)),
  );

  const deduped = new Map();
  for (const article of [...articles, ...searchFallbackArticles]) {
  ...
  const videos = await getCuratedReactorVideos(
    `${leagueQuery || "Premier League"} ${seasonLabel || ""} highlights goals`,
    { limit: videoLimit },
  );
  ...
  return {
    ...
    videos,
    providers: {
      articles: searchFallbackArticles.length
        ? "rss-google-og-enrichment"
        : "rss-og-enrichment",
