...
function legacyProgressiveUpdate({
  updateId,
  resource,
  resourceId,
  operation,
  ...
  const pocReached = STAGE_INDEX.get(stage) >= STAGE_INDEX.get(APU_STAGES.POC);
...
export function normalizeApuProgressiveUpdate(value) {
  ...
  return {
    ...
    ...(value.swfus_receipt ? { swfus_receipt: value.swfus_receipt } : {}),
  };
}
...
export function bindApuProgressivePayload(value, { payload, idempotencyKey } = {}) {
  ...
  if (STAGE_INDEX.get(current.stage) > STAGE_INDEX.get(APU_STAGES.IMPLEMENTED)) {
    throw new Error("APU payload may only be bound before the POC persistence transition.");
  }
  return normalizeApuProgressiveUpdate({
    ...current,
    progressive_update: {
      ...current.progressive_update,
      value: payload,
      idempotency_key: idempotencyKey || current.progressive_update.idempotency_key,
    },
  });
}
...
export function markApuQueuePoc(value, { receiptId, at = new Date().toISOString() } = {}) {
  ...
  return promoteApuProgressiveUpdate(
    {
      ...current,
      progressive_update: {
        ...current.progressive_update,
        poc_validated: true,
        evidence_refs: [...new Set([...current.progressive_update.evidence_refs, evidenceRef])],
      },
    },
    APU_STAGES.POC,
    {
