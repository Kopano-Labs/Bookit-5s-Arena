...
export async function enqueueOfflineEvent({
  ...
  const now = Date.now();
  const persistedApu = apu
    ? markApuQueuePoc(normalizeApuProgressiveUpdate(apu), {
  ...
  await putQueueRecord(db, record, {
    ...
    idempotency_key: idempotencyKey,
    event_type: eventType,
    apu_update_id: persistedApu?.update_id || null,
    apu_stage: persistedApu?.stage || null,
  });
