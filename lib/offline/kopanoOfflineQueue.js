...
function defaultApuForOfflineEvent(eventType, idempotencyKey) {
  return createApuProgressiveUpdate({
    updateId: `${eventType}:apu:${idempotencyKey}`,
    resource: `${eventType}-intent`,
    operation: "create",
    idempotencyKey,
...
export async function enqueueOfflineEvent({
  ...
  const now = Date.now();
  const implementedApu = apu
    ? normalizeApuProgressiveUpdate(apu)
    : defaultApuForOfflineEvent(eventType, idempotencyKey);
  const boundApu = bindApuProgressivePayload(implementedApu, {
    payload,
    idempotencyKey,
  });
  const persistedApu = markApuQueuePoc(boundApu, {
    receiptId: `queue:${idempotencyKey}`,
  ...
  await putQueueRecord(db, record, {
    ...
    idempotency_key: idempotencyKey,
    event_type: eventType,
    apu_update_id: persistedApu.update_id,
    apu_stage: persistedApu.stage,
    kpgs_boundary_marker: persistedApu.progressive_update.boundary_marker,
  });
...
async function markRecord(db, record, patch, auditType) {
  ...
  await putQueueRecord(db, updated, {
    ...
    apu_update_id: updated.apu?.update_id || null,
    apu_stage: updated.apu?.stage || null,
  });

  return updated;
}

async function deleteRecord(db, record, syncedApu = null) {
  const transaction = db.transaction([STORES.QUEUE, STORES.AUDIT], "readwrite");
  ...
  transaction.objectStore(STORES.AUDIT).add({
    ...
    idempotency_key: record.idempotency_key,
    event_type: record.event_type,
    apu_update_id: syncedApu?.update_id || record.apu?.update_id || null,
    apu_stage: syncedApu?.stage || record.apu?.stage || null,
    swfus_disposition: syncedApu?.swfus_receipt?.disposition || null,
    at: Date.now(),
  });
  await transactionDone(transaction);
}
  ...
  for (const record of records) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (record.apu) {
          if (!responseBody?.apu) {
            throw new Error("APU synchronization response is missing its server proof receipt.");
          }
          syncedApu = normalizeApuProgressiveUpdate(responseBody.apu);
          if (syncedApu.stage !== APU_STAGES.SYNCED) {
            throw new Error(`APU synchronization did not reach ${APU_STAGES.SYNCED}.`);
          }
          if (syncedApu.update_id !== record.apu.update_id) {
            throw new Error("APU synchronization receipt does not match the queued update_id.");
          }
          if (
            syncedApu.swfus_receipt?.disposition !== "APPLIED" ||
            syncedApu.swfus_receipt?.synchronized !== true
          ) {
            throw new Error("APU synchronization receipt is missing an APPLIED SWFUS consequence.");
          }
        }

        await deleteRecord(db, syncingRecord, syncedApu);
        synced += 1;
        continue;
      }
