...
import { canAssumeRole, defaultActiveRole, normalizeRoles } from '@/lib/roles';
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        return {
          ...
          name: user.name,
          email: user.email,
          role: defaultActiveRole(user.email, user.roles || [user.role || 'user']),
          roles: normalizeRoles(user.email, user.roles || [user.role || 'user']),
          activeRole: defaultActiveRole(user.email, user.roles || [user.role || 'user']),
  ...
  callbacks: {
    ...
    async jwt({ token, user, account, trigger, session: updateSession }) {
      ...
      if (trigger === 'update' && updateSession) {
        ...
        // Role switching: allow the requested role only if the account can actually assume it.
        if (updateSession.activeRole !== undefined) {
          if (canAssumeRole(token.email, currentRoles, updateSession.activeRole)) {
            token.activeRole = updateSession.activeRole;
            token.role = updateSession.activeRole;
          }
        }
        if (!canAssumeRole(token.email, currentRoles, token.activeRole)) {
          const fallbackRole = defaultActiveRole(token.email, currentRoles);
          token.activeRole = fallbackRole;
      ...
      if (user) {
        const roles = normalizeRoles(user.email, user.roles || [user.role || 'user']);
        const fallbackRole = defaultActiveRole(user.email, roles);
        token.id = user.id;
        token.roles = roles;
        token.activeRole = fallbackRole;
        token.role = fallbackRole;
      ...
      if (account?.provider === 'google' || account?.provider === 'facebook') {
        ...
        token.roles = normalizeRoles(dbUser.email, dbUser.roles || [dbUser.role || 'user']);
        token.activeRole = defaultActiveRole(dbUser.email, token.roles);
      ...
      if (!user && !account && trigger !== 'update' && token.id) {
        const hasValidPic = token.picture && (String(token.picture).startsWith('/') || String(token.picture).startsWith('http'));
        if (!hasValidPic) {
          try {
            await connectDB();
            ...
            console.warn('[JWT_DB_LOOKUP_FAILED] Proceeding with cached token.');
          }
        }
      }

      token.roles = normalizeRoles(token.email, token.roles || [token.role || 'user']);
      if (!canAssumeRole(token.email, token.roles, token.activeRole)) {
        token.activeRole = defaultActiveRole(token.email, token.roles);
        token.role = token.activeRole;
      }
    ...
    async session({ session, token }) {
      if (token) {
        ...
        const activeRole = canAssumeRole(token.email, roles, token.activeRole)
          ? token.activeRole
          : defaultActiveRole(token.email, roles);
        session.user.id = token.id;
        session.user.roles = roles;
        session.user.activeRole = activeRole;
        session.user.role = activeRole; // Backward compat alias
