...
export async function getPremierLeagueStats(seasonYear, categoryKey = "goals") {
  ...
  if (selectedSeason.year !== activeSeasonYear) {
    const activeSeason =
      seasonOptions.find((option) => option.year === activeSeasonYear) ||
      seasonOptions[0];
    const activePayload = await getPremierLeagueStats(activeSeasonYear, categoryKey);

    return {
      ...activePayload,
      season: selectedSeason,
      displaySeason: activeSeason,
      seasonNotice: `${selectedSeason.label} leaderboards are not published yet. Showing live ${activeSeason.label} stats.`,
    };
  }
