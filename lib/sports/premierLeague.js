...
async function getFplFallbackMatches(seasonYear, referenceDate = new Date()) {
  ...
  if (seasonYear !== activeSeasonYear) {
    const nextLabel = `${activeSeasonYear}-${String(activeSeasonYear + 1).slice(-2)}`;
    return {
      status: "coming-soon",
      provider: "fpl",
      matches: [],
      emptyState:
        `Fixtures for that season are not live yet. Switch to the current ${nextLabel} season to see this week's matches.`,
    };
  }
...
export async function getPremierLeagueMatches(seasonYear) {
  ...
  try {
    ...
    if (isportsResult.matches.length > 0) {
      return {
    ...
    // Fall through to FPL when iSports is empty or unavailable.
  }
