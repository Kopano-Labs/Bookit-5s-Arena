...
function finish(update, disposition, stages, { synchronized = false, state = null } = {}) {
  return {
...
export function evaluateKpgsProgressiveUpdate(value, currentProjection = null) {
  ...
  if (update.operation === "READ") {
    ...
    return {
      update,
      receipt: finish(update, SWFUS_DISPOSITIONS.OBSERVED, stages, { state: currentProjection }),
  ...
  if (invariantFailures.length > 0) {
    ...
    return {
      update,
      receipt: finish(update, SWFUS_DISPOSITIONS.REJECTED, stages, { state: currentProjection }),
      nextProjection: currentProjection,
      distribution: null,
    };
  }
  ...
  if (update.apu_status === "YELLOW") {
    ...
    return {
      update,
      receipt: finish(update, SWFUS_DISPOSITIONS.HELD, stages, { state: currentProjection }),
      nextProjection: currentProjection,
      distribution: null,
    };
  }
  if (!update.poc_validated || update.evidence_refs.length === 0) {
    stages.push(
      stage("POC_FOC_CHECK", "HOLD", "mutation requires POC validation and evidence refs"),
    );
    return {
      update,
      receipt: finish(update, SWFUS_DISPOSITIONS.HELD, stages, { state: currentProjection }),
  ...
  return {
    update,
    receipt: finish(update, SWFUS_DISPOSITIONS.APPLIED, stages, {
      synchronized: true,
      state: nextProjection,
    }),
    nextProjection,
    distribution,
  };
