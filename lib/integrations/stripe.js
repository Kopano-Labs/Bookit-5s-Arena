...
// Lazy-load Stripe to avoid errors when package isn't installed
let stripe = null;
const getStripe = () => {
  if (!stripe) {
    try {
...
export async function createCheckoutSession(booking) {
  ...
  const session = await s.checkout.sessions.create({
    ...
    mode: 'payment',
    success_url: `${process.env.NEXTAUTH_URL}/bookings?payment=success&session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${process.env.NEXTAUTH_URL}/bookings?payment=cancelled`,
