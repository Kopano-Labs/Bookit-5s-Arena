...
function NewsGrid({ articles, videos }) {
  return (
    <div className="space-y-10 px-4 py-8">
      {videos?.length > 0 && (
        <section>
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-400 mb-6">Trending Highlights</h3>
            ...
            {videos.map((video) => (
              <a key={video.id} href={video.url} target="_blank" rel="noreferrer" className="group block space-y-3">
                <div className="relative aspect-video rounded-3xl overflow-hidden bg-zinc-100">
                  <Image src={video.thumbnail} alt={video.title} fill className="object-cover transition group-hover:scale-105" unoptimized />
                  <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition flex items-center justify-center">
                    <div className="w-12 h-12 bg-white/90 rounded-full flex items-center justify-center shadow-xl group-hover:scale-110 transition">
                      <FaPlay className="text-zinc-950 ml-1" size={16} />
                    </div>
                  </div>
                </div>
                <div>
                  <h4 className="text-sm font-bold text-zinc-900 line-clamp-2 leading-snug">{video.title}</h4>
                  <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mt-1">{video.reactor?.name || video.channelName}</p>
                </div>
              </a>
            ))}
          </div>
      ...
      {articles?.length > 0 && (
        <section>
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-400 mb-6">Latest Intelligence</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {articles.map((article) => (
              <a key={article.url} href={article.url} target="_blank" rel="noreferrer" className="flex gap-4 p-4 rounded-3xl border border-zinc-200 bg-white hover:border-violet-200 hover:shadow-lg transition group">
                {article.image && (
                ...
                <div className="flex flex-col justify-between py-1">
                  <div>
                    <h4 className="text-sm font-bold text-zinc-950 line-clamp-2 leading-snug group-hover:text-violet-600 transition">{article.title}</h4>
                    <p className="text-[11px] text-zinc-500 line-clamp-2 mt-1">{article.summary}</p>
                  </div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mt-2">{article.source}</span>
                </div>
              </a>
            ))}
          </div>
...
export default function FootballFixturesHub({ slug = "premier-league" }) {
  ...
  if (loading) {
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center gap-4 bg-white rounded-[32px] border border-zinc-200">
  ...
  if (error) {
    return (
      <div className="p-8 text-center bg-white rounded-[32px] border border-zinc-200">
        <h3 className="text-lg font-black text-zinc-950 uppercase tracking-widest mb-2">Network Isolated</h3>
        <p className="text-zinc-500 text-sm mb-6">{error}</p>
        <button onClick={() => window.location.reload()} className="px-6 py-3 bg-zinc-950 text-white font-black uppercase tracking-widest text-[10px] rounded-xl">
          Retry Sync
        </button>
      </div>
    );
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between px-2">
        <div className="flex items-center gap-5">
          <div className="relative h-16 w-16 overflow-hidden rounded-[22px] bg-white p-3 shadow-lg ring-1 ring-black/5">
            <Image src={meta.league.logo} alt={meta.league.name} fill className="object-contain p-2" unoptimized />
          </div>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-zinc-950 uppercase">{meta.league.name}</h1>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Powered by iSports</span>
            </div>
          </div>
        </div>
        
        <div className="flex p-1 bg-zinc-100 rounded-2xl border border-zinc-200 self-start">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${
                activeTab === tab.key ? "bg-white text-zinc-950 shadow-sm" : "text-zinc-500 hover:text-zinc-700"
              }`}
            >
              <tab.icon size={11} />
              {tab.label}
      ...
      <div className="overflow-hidden rounded-[32px] border border-zinc-200 bg-white shadow-xl shadow-zinc-200/50">
          ...
          {activeTab === "matches" ? (
            ...
            >
              <div className="bg-zinc-50/80 px-6 py-4 border-b border-zinc-200 flex items-center justify-between">
                  <h2 className="text-sm font-black uppercase tracking-widest text-zinc-950">Active Match Window</h2>
              ...
              {matchesPayload?.groups?.length ? (
                <div className="divide-y divide-zinc-200">
                  {matchesPayload.groups.map((group) => (
                    <div key={group.dateKey}>
                      <div className="bg-zinc-50/50 px-4 py-3 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 sm:px-6">
                        {group.dateLabel}
                      </div>
                      <div>
                ...
                <div className="py-20 text-center">
                  <FaTrophy className="mx-auto text-zinc-200 mb-4" size={48} />
            ...
            >
              {newsLoading ? (
                <div className="py-20 flex flex-col items-center gap-4">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 animate-pulse">Scanning Global Feeds...</p>
                </div>
              ) : (
