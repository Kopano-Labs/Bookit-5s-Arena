...
const RARITY_STYLES = {
  ...
  uncommon:  { border: 'border-green-700/60',glow: 'shadow-[0_0_12px_rgba(34,197,94,0.2)]',  badge: 'bg-green-900/40 text-green-400',  label: 'Uncommon'  },
  rare:      { border: 'border-blue-700/60', glow: 'shadow-[0_0_12px_rgba(96,165,250,0.2)]',  badge: 'bg-blue-900/40 text-blue-400',    label: 'Rare'      },
  epic:      { border: 'border-purple-700/60',glow:'shadow-[0_0_14px_rgba(167,139,250,0.25)]',badge: 'bg-purple-900/40 text-purple-400', label: 'Epic'      },
  legendary: { border: 'border-yellow-600/60',glow:'shadow-[0_0_18px_rgba(245,158,11,0.3)]',  badge: 'bg-yellow-900/40 text-yellow-400', label: 'Legendary' },
};

const TIER_COLORS = {
  Bronze:  { from: '#92400e', to: '#d97706', text: 'text-amber-500',  bg: 'bg-amber-900/20',   border: 'border-amber-700/40'  },
  Silver:  { from: '#475569', to: '#94a3b8', text: 'text-slate-300',  bg: 'bg-slate-800/40',   border: 'border-slate-600/40'  },
  Gold:    { from: '#92400e', to: '#f59e0b', text: 'text-yellow-400', bg: 'bg-yellow-900/20',  border: 'border-yellow-700/40' },
  Diamond: { from: '#1e40af', to: '#60a5fa', text: 'text-blue-400',   bg: 'bg-blue-900/20',    border: 'border-blue-700/40'   },
};
...
export default function RewardsPage() {
      ...
      <div className="min-h-screen bg-gray-950 py-16 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="inline-flex items-center justify-center w-24 h-24 rounded-full border-2 border-yellow-500 mb-6 shadow-[0_0_40px_rgba(245,158,11,0.3)]"
              style={{ background: 'radial-gradient(circle, rgba(245,158,11,0.2) 0%, transparent 70%)' }}>
              <FaTrophy className="text-4xl text-yellow-400" />
            </div>
            <h1 className="text-4xl font-black uppercase tracking-widest text-white mb-3" style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
              5S Rewards
            </h1>
            <p className="text-gray-400 mb-8 max-w-md mx-auto leading-relaxed">
              Earn points, unlock achievements, climb tiers and claim exclusive perks every time you step on the pitch.
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
              {[
                ...
                <motion.div key={t.label} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 + 0.3 }}
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link href="/register"
                ...
                style={{ background: 'linear-gradient(135deg, #15803d 0%, #22c55e 100%)', boxShadow: '0 0 25px rgba(34,197,94,0.4)' }}>
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-yellow-400 animate-pulse font-semibold">Loading your rewards...</p>
        </div>
      </div>
    );
  }
  ...
  const progressPct = tier.nextAt
    ...
    : 100;
  const bookingsUntilNext = tier.nextAt ? Math.max(tier.nextAt - data.totalBookings, 0) : 0;
  ...
  return (
    <div className="min-h-screen bg-gray-950 py-10 px-4">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        >
          {/* Background glow */}
          <div className="absolute inset-0 opacity-10" style={{ background: `radial-gradient(ellipse at top right, ${tier.color} 0%, transparent 60%)` }} />
          ...
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-gray-400 text-xs uppercase tracking-widest mb-0.5 flex items-center gap-1">
                  Current Tier <InfoTooltip text="Tiers: Bronze (0+) → Silver (5 bookings) → Gold (10) → Diamond (20). Higher tiers unlock better perks and priority booking." position="right" />
                </p>
                <h1 className={`text-3xl font-black uppercase tracking-widest ${tierStyle.text}`} style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
                  {tier.name}
                </h1>
            ...
            <div className="text-right">
              <p className="text-gray-500 text-xs uppercase tracking-widest flex items-center justify-end gap-1">
                Arena Points <InfoTooltip text="Earn 100 points per confirmed booking + 50 points per hour played. Points determine your tier and unlock exclusive perks." position="left" />
              </p>
              <motion.p
          ...
          {tier.next && (
            <div className="relative mt-5">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs text-gray-500 uppercase tracking-widest">{tier.name}</span>
                <span className="text-xs font-bold text-gray-300">
                  {bookingsUntilNext > 0
                    ? `${bookingsUntilNext} booking${bookingsUntilNext !== 1 ? 's' : ''} to ${tier.next}`
                    : `${tier.next} unlocked!`}
                </span>
                <span className={`text-xs uppercase tracking-widest ${tierStyle.text}`}>{tier.next}</span>
              </div>
              <div className="h-3 bg-gray-800 rounded-full overflow-hidden">
                <motion.div
                  ...
                  style={{ background: `linear-gradient(90deg, ${tier.color}, ${TIER_COLORS[tier.next]?.to || '#22c55e'})` }}
                />
              </div>
          ...
          {activeTab === 'overview' && (
            ...
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <StatCard icon={<FaFutbol />} label="Courts Booked" value={data.totalBookings} color="text-green-400" delay={0} tooltip="Total confirmed bookings" />
                <StatCard icon={<FaClock />} label="Hours Played" value={`${data.totalHours}h`} color="text-blue-400" delay={0.07} tooltip="Total hours on the pitch" />
                <StatCard icon={<FaMoneyBillWave />} label="Total Spent" value={`R${data.totalSpent.toLocaleString()}`} color="text-yellow-400" delay={0.14} tooltip="Total confirmed spend" />
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setActiveTab('achievements')} className="text-xs text-green-400 hover:text-green-300 transition-colors flex items-center gap-1">
                ...
                <div className="flex items-center gap-2 mb-3">
                  <div className="flex-1 h-2 bg-gray-800 rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${(data.unlockedCount / data.achievements.length) * 100}%` }}
                      transition={{ duration: 1, delay: 0.3 }}
                      className="h-full rounded-full bg-gradient-to-r from-yellow-600 to-yellow-400"
                    />
                  </div>
                  <span className="text-xs text-gray-400 font-bold flex-shrink-0">{data.unlockedCount}/{data.achievements.length}</span>
                </div>
              ...
              {tier.next && bookingsUntilNext <= 3 && (
                ...
                >
                  <FaFire className="text-orange-400 text-xl mt-0.5 flex-shrink-0 animate-pulse" />
                  <div>
                    <p className="text-white font-bold text-sm">You&apos;re so close!</p>
                    <p className="text-gray-400 text-xs mt-0.5">
                      Just {bookingsUntilNext} more booking{bookingsUntilNext !== 1 ? 's' : ''} and you&apos;ll reach{' '}
                      <span className={tierStyle.text}>{tier.next}</span> tier — unlock exclusive benefits!
                    </p>
                    <Link href="/" className="inline-flex items-center gap-1 mt-2 text-xs text-green-400 hover:text-green-300 font-bold transition-colors">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {data.achievements.map((a, i) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-14 h-14 rounded-xl flex items-center justify-center text-3xl flex-shrink-0 ${a.unlocked ? 'bg-gray-800' : 'bg-gray-900'} border border-gray-700`}>
                        {a.unlocked ? a.icon : '🔒'}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-0.5">
                          <p className="text-white font-bold text-sm">{a.name}</p>
                          <span className={`text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ${rs.badge}`}>
                            {rs.label}
                          </span>
                        </div>
            ...
            >
              {data.recent.length === 0 ? (
                <div className="bg-gray-900 border border-gray-800 rounded-2xl p-12 text-center">
                  ...
                  <Link href="/" className="inline-flex items-center gap-2 mt-4 py-3 px-6 rounded-xl text-sm font-black text-white uppercase tracking-widest hover:scale-105 transition-all"
                ...
                <>
                  {data.recent.map((b, i) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-green-900/30 border border-green-800/50 flex items-center justify-center flex-shrink-0">
                          <FaFutbol className="text-green-400 text-sm" />
                        </div>
                        <div>
                          <p className="text-white font-bold text-sm">{b.courtName}</p>
                          <p className="text-gray-500 text-xs">{b.date} · {b.start_time} · {b.duration}h</p>
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="text-green-400 font-black text-sm">R{b.total_price}</p>
                  ...
                  <Link href="/bookings" className="block text-center text-xs text-green-400 hover:text-green-300 transition-colors py-2">
                    View all bookings →
                  </Link>
                </>
              )}
              ...
              {[
                {
                  ...
                  color: tierStyle,
                  perks: [
                    { icon: '📅', title: 'Online Booking', desc: 'Reserve courts 24/7 from anywhere', active: true },
                    { icon: '📧', title: 'Booking Confirmations', desc: 'Instant email confirmations for every booking', active: true },
                    { icon: '📍', title: 'Pay at Venue', desc: 'Reserve now and pay cash or card on arrival', active: true },
                  ],
                },
                {
                  tier: 'Silver & Above',
                  ...
                  perks: [
                    { icon: '⚡', title: 'Priority Booking', desc: 'Book up to 7 days in advance (vs 3 days standard)', active: data.totalBookings >= 5 },
                    { icon: '🎁', title: 'Member Discounts', desc: '5% off on bookings of 2+ hours', active: data.totalBookings >= 5 },
                  ],
                },
                {
                  tier: 'Gold & Above',
                  ...
                  perks: [
                    { icon: '🥇', title: 'Gold Booking Window', desc: '14-day advance booking window', active: data.totalBookings >= 10 },
                    { icon: '💰', title: 'Loyalty Discount', desc: '10% off all bookings', active: data.totalBookings >= 10 },
                    { icon: '🎉', title: 'Birthday Surprise', desc: 'Special offer on your birthday month', active: data.totalBookings >= 10 },
                  ],
                },
                {
                  tier: 'Diamond',
                  ...
                  perks: [
                    { icon: '💎', title: 'VIP Status', desc: 'Your name on the Wall of Legends', active: data.totalBookings >= 20 },
                    { icon: '🆓', title: 'Free Hour', desc: 'One complimentary hour per month', active: data.totalBookings >= 20 },
                    { icon: '📞', title: 'Dedicated Line', desc: 'Direct WhatsApp line for bookings', active: data.totalBookings >= 20 },
                  ],
                },
              ].map((section, si) => (
                <motion.div
                ...
                >
                  <div className={`px-5 py-3 border-b border-gray-800 flex items-center justify-between ${section.color.bg || 'bg-gray-800/30'}`}>
                    <h3 className={`text-sm font-black uppercase tracking-widest ${section.color.text || 'text-white'}`}>
                      {section.tier}
                    </h3>
                    {section.locked && (
                      <span className="flex items-center gap-1.5 text-xs text-gray-500">
                        <FaLock size={9} /> {data.totalBookings < 5 ? '5' : data.totalBookings < 10 ? '10' : '20'} bookings to unlock
                        <InfoTooltip text="Keep booking courts to reach this tier. Every confirmed booking counts toward your total." position="left" size={11} />
                      </span>
                    )}
                  </div>
                  <div className="divide-y divide-gray-800">
                    {section.perks.map((perk) => (
                      <div key={perk.title} className={`flex items-start gap-3 px-5 py-3.5 ${!perk.active ? 'opacity-40' : ''}`}>
                        ...
                        {perk.active && <span className="ml-auto text-green-400 text-xs font-bold flex-shrink-0">✓ Active</span>}
                      </div>
                    ))}
                  </div>
                </motion.div>
