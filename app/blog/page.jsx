...
import { 
  FaLayerGroup, FaTags, FaTrophy, FaTools, FaPlay, FaCalendarAlt, FaUser,
  FaBolt, FaArrowRight, FaVideo, FaNewspaper
} from 'react-icons/fa';
...
const SERIES = [
  ...
  { id: 'tournament', name: 'Competition News', icon: FaTrophy, color: 'text-yellow-400' },
  { id: 'highlights', name: 'Match Highlights', icon: FaVideo, color: 'text-red-400' },
...
const POSTS = [
  {
    ...
    date: '14 April 2026',
    author: 'Engineering',
    excerpt: 'A deep dive into our Next.js 15, Framer Motion, and Tailwind CSS tech stack for the ultimate football booking experience.',
  ...
  },
  {
    ...
    author: 'Media Team',
    excerpt: 'Watch the most clinical finishes and acrobatic saves from the 5s Arena 2026 Season opening night.',
    link: '#',
...
export default function BlogIndexPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white selection:bg-green-500 selection:text-black overflow-x-hidden">
      ...
      <meta name="description" content="Stay updated with 5s Arena tournament results, player highlights, and venue news. The ultimate source for Cape Town 5-a-side football intel." />

      {/* ── DECORATIVE BACKGROUND ── */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(34,197,94,0.08)_0%,transparent_50%)]" />
        {Array.from({ length: 20 }).map((_, i) => <HubParticle key={i} idx={i} />)}
      </div>
      ...
      <div className="relative z-10 pt-24 pb-20 px-4 md:px-8 max-w-7xl mx-auto">
        ...
        <section className="mb-16">
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12 flex flex-col items-start max-w-3xl">
              <span className="px-3 py-1 bg-green-600 text-white text-[10px] font-black uppercase tracking-widest rounded-lg mb-4">Featured Report</span>
              <h2 className="text-3xl md:text-5xl font-black uppercase tracking-widest text-white mb-4 group-hover:text-green-400 transition-colors leading-none" style={{ fontFamily: 'Impact, sans-serif' }}>
                {featuredPost.title}
              </h2>
              ...
              >
                Access Full Intel <FaArrowRight className="group-hover/btn:translate-x-1 transition-transform" />
              </Link>
            </div>
            ...
            <div className="absolute top-8 right-8 backdrop-blur-md bg-white/5 border border-white/10 p-4 rounded-2xl">
              <div className="flex flex-col items-end">
                <span className="text-[10px] font-black uppercase text-white/40 tracking-widest">Released</span>
                <span className="text-xs font-bold text-white uppercase">{featuredPost.date}</span>
              </div>
            </div>
          </motion.div>
        </section>
           ...
           <div className="flex flex-wrap justify-center gap-3 p-2 bg-gray-900/60 backdrop-blur-xl border border-gray-800 rounded-3xl shadow-2xl">
            {SERIES.map(series => {
              ...
              return (
                ...
                >
                  <Icon className={`${isActive ? 'text-black' : series.color}`} size={14} /> {series.name}
                </button>
              )
            })}
          </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <AnimatePresence mode="popLayout">
            {filteredPosts.filter(p => !p.featured || activeSeries !== 'all').map((post, i) => (
              <motion.div
                layout
                key={post.id}
                initial={{ opacity: 0, scale: 0.9 }}
              ...
              >
                ...
                <div className="p-6 md:p-8 flex-1 flex flex-col">
                  ...
                  <Link href={post.link}>
                    <h3 className="text-xl font-black uppercase tracking-widest text-white mb-4 group-hover:text-green-500 transition-colors leading-tight min-h-[3rem]" style={{ fontFamily: 'Impact, sans-serif' }}>
                      {post.title}
                    </h3>
                  </Link>
                  ...
                  <div className="mt-auto">
                    ...
                    >
                      Initialize Link <FaArrowRight className="group-hover:translate-x-1 transition-transform" />
                    </Link>
                  </div>
                </div>
        ...
        <section className="mt-20 py-20 bg-green-950/10 border border-green-900/20 rounded-[3rem] text-center px-4 relative overflow-hidden">
           ...
           <h4 className="text-3xl md:text-5xl font-black uppercase tracking-widest mb-4" style={{ fontFamily: 'Impact, sans-serif' }}>
             NEVER MISS A <span className="text-green-500">MOMENT</span>
           </h4>
           <p className="text-gray-400 text-sm max-w-md mx-auto mb-10">
             Join 2,500+ fans receiving weekly tactical breakdowns and arena scouting reports directly in their inbox.
           </p>
