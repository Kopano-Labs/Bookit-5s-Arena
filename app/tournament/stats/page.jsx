...
import {
  FaTrophy, FaUsers, FaFutbol, FaCalendarAlt,
  FaChartBar, FaArrowRight, FaChevronDown,
} from 'react-icons/fa';
import Link from 'next/link';

const STAT_CARDS = [
  { label: 'Teams Registered', icon: FaUsers, color: '#22c55e', key: 'registeredCount' },
...
export default function TournamentStatsPage() {
  ...
  const now = new Date();
  const daysUntilDeadline = deadline ? Math.max(0, Math.ceil((deadline - now) / (1000 * 60 * 60 * 24))) : null;
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <section className="py-20 text-center px-6">
        ...
        >
          ...
          <p className="text-gray-400 max-w-lg mx-auto">
            Live registration stats and tournament information for the 5s Arena World Cup 2026.
          </p>
        </motion.div>
      </section>
            ...
            return (
              ...
              >
                ...
                <p
                  ...
                  style={{ fontFamily: 'Impact, Arial Black, sans-serif', color: card.color }}
                >
                  {loading ? '...' : value}
                </p>
      ...
      {daysUntilDeadline !== null && (
        <section className="max-w-4xl mx-auto px-6 pb-12">
          ...
          >
            ...
            {data?.isOpen ? (
              <>
                <p className="text-5xl font-black text-green-400 mb-2" style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
                  {daysUntilDeadline} <span className="text-xl text-gray-500">DAYS</span>
                </p>
                <p className="text-gray-500 text-sm">
                  Deadline: {deadline?.toLocaleDateString('en-ZA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                </p>
              </>
            ) : (
              <p className="text-red-400 text-lg font-bold">Registration has closed. Good luck to all teams!</p>
            )}
          </motion.div>
        </section>
      )}
      ...
      <section className="max-w-4xl mx-auto px-6 pb-12">
        ...
        >
          <div className="flex justify-between items-center mb-3">
            <span className="text-xs font-bold uppercase tracking-widest text-gray-400">Tournament Capacity</span>
          ...
          <div className="h-3 bg-gray-800 rounded-full overflow-hidden">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{ width: loading ? '0%' : `${((data?.registeredCount || 0) / 48) * 100}%` }}
      ...
      <section className="max-w-4xl mx-auto px-6 pb-12">
        ...
        >
          <h3 className="text-green-400 text-xs font-bold uppercase tracking-widest mb-4">Tournament Format</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-400">
            {[
              ...
              <div key={label} className="flex items-start gap-2">
                <FaFutbol size={10} className="text-green-500 mt-1 flex-shrink-0" />
                <div>
