...
import {
  FaBirthdayCake, FaStar, FaBuilding, FaTrophy,
  FaCalendarAlt, FaClock, FaUsers, FaEnvelope, FaPhone, FaUser,
  FaCheck, FaArrowLeft, FaCalendarCheck, FaInfoCircle,
} from 'react-icons/fa';
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
        ...
        >
          ...
          <h2 className="text-2xl font-black text-white uppercase tracking-wider mb-3" style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
            Booking Confirmed!
          </h2>
    ...
    <div className="min-h-screen bg-gray-950 py-10 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="mb-6">
          <Link href="/events" className="inline-flex items-center gap-2 text-gray-400 hover:text-white text-sm font-bold uppercase tracking-widest transition-colors">
        ...
        <motion.div className="text-center mb-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h1 className="text-3xl md:text-5xl font-black text-white uppercase tracking-widest mb-2" style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
            Book Your <span className="text-green-400">Event</span>
          </h1>
          <p className="text-gray-500 text-sm">Choose a package, pick your date, and we&apos;ll handle the rest.</p>
        </motion.div>
        ...
        {!selectedPkg ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
              <span className="w-6 h-6 bg-green-600 rounded-full flex items-center justify-center text-white text-[10px] font-black">1</span>
              Choose Your Package
            </h2>
                  ...
                  >
                    <div className="relative h-32 overflow-hidden">
                      ...
                      <div className="absolute bottom-3 left-4 flex items-center gap-2">
                        <div className={`w-10 h-10 bg-gradient-to-br ${pkg.color} rounded-lg flex items-center justify-center`}>
                          <Icon className="text-white text-lg" />
                        </div>
                        <div>
                          <h3 className="text-white font-black text-sm uppercase tracking-wider">{pkg.name}</h3>
                          <p className="text-gray-400 text-[10px]">{pkg.duration} &bull; Up to {pkg.maxGuests} guests</p>
                        </div>
                      </div>
                    </div>
                  </motion.button>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden sticky top-24">
                <div className="relative h-40 overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4">
                    <h3 className="text-white font-black text-lg uppercase tracking-wider" style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
                      {selectedPkg.name}
                    </h3>
                  </div>
                </div>
                <div className="p-5">
                  <div className="flex items-center gap-4 mb-4 text-sm text-gray-400">
                    <span className="flex items-center gap-1"><FaClock size={11} /> {selectedPkg.duration}</span>
                    <span className="flex items-center gap-1"><FaUsers size={11} /> Up to {selectedPkg.maxGuests}</span>
                  </div>
                  <ul className="space-y-2">
                    {selectedPkg.features.map((f, i) => (
                      <motion.li key={i} className="flex items-start gap-2 text-gray-400 text-sm"
                        initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                        <FaCheck className="text-green-400 text-[10px] mt-1.5 flex-shrink-0" />
                        {f}
                      </motion.li>
                    ))}
                  </ul>
                  <button onClick={() => setSelectedPkg(null)} className="mt-4 text-[10px] text-gray-600 hover:text-gray-400 uppercase tracking-widest font-bold transition-colors cursor-pointer">
                    ← Change Package
                  </button>
                </div>
              </div>
            ...
            >
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
                <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                  <span className="w-6 h-6 bg-green-600 rounded-full flex items-center justify-center text-white text-[10px] font-black">2</span>
                  Event Details
                </h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-widest flex items-center gap-1"><FaUser size={9} /> Full Name</label>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-widest flex items-center gap-1"><FaPhone size={9} /> Phone</label>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-widest flex items-center gap-1"><FaEnvelope size={9} /> Email</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-widest flex items-center gap-1"><FaCalendarAlt size={9} /> Date</label>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-widest flex items-center gap-1"><FaClock size={9} /> Time</label>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-widest flex items-center gap-1"><FaUsers size={9} /> Guests</label>
                      <input type="number" name="guests" value={form.guests} onChange={handleChange} required min="1" max={selectedPkg.maxGuests}
                        className="w-full px-4 py-3 bg-gray-800/50 border border-gray-700/50 rounded-xl text-white text-sm focus:ring-2 focus:ring-green-500/50 focus:border-green-600/50 outline-none transition-all"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-widest">Special Requests</label>
