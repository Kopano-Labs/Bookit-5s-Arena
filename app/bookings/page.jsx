...
function TabButton({ tab, isActive, onClick, tabIndex: tIdx, activeIndex }) {
  ...
  const colorMap = {
    green: { active: '#22c55e', bg: 'rgba(34,197,94,0.12)', border: 'rgba(34,197,94,0.5)' },
    amber: { active: '#f59e0b', bg: 'rgba(245,158,11,0.12)', border: 'rgba(245,158,11,0.5)' },
    blue:  { active: '#3b82f6', bg: 'rgba(59,130,246,0.12)', border: 'rgba(59,130,246,0.5)' },
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center py-4 px-3 gap-1">
        ...
        <span className="text-[9px] text-gray-600 hidden sm:block">{tab.subtitle}</span>
      </div>
    </motion.button>
  );
}
...
function CourtBookCard({ court, index }) {
  return (
    ...
    >
      <Link href={`/courts/${court._id}`} className="group block relative overflow-hidden rounded-2xl border border-gray-800 bg-gray-900 hover:border-green-500/50 hover:shadow-[0_0_30px_rgba(34,197,94,0.2)] transition-all duration-300">
        <div className="relative h-44 overflow-hidden">
          {court.image ? (
            <motion.img
              ...
              transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut', repeatType: 'loop' }}
            />
          ) : (
          ...
          <div className="absolute bottom-3 left-3">
            <h3 className="text-white font-black text-base uppercase tracking-wider drop-shadow" style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
              {court.name}
            </h3>
          </div>
...
function EventBookCard({ card, index }) {
  return (
    ...
    >
      ...
      {/* Header gradient */}
      <div className={`relative h-28 bg-gradient-to-br ${card.gradient} flex items-center justify-center overflow-hidden`}>
        <motion.span
          className="text-5xl"
      ...
      <div className="p-5">
        <h3 className="text-white font-black text-lg uppercase tracking-wider mb-3" style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
          {card.name}
        </h3>
...
function MyBookingsTab({ bookings, loading, error, authStatus, viewMode, setViewMode, handleCancel, showTutorial, setShowTutorial, neverShowAgain, setNeverShowAgain }) {
                  ...
                  <div className="flex flex-col items-end gap-3 flex-shrink-0">
                    ...
                    <div className="flex gap-2">
                      ...
                      {booking.status !== 'cancelled' && (
                        <button onClick={() => handleCancel(booking._id)} className="text-xs px-3 py-2 bg-red-950 border border-red-900 rounded-xl text-red-400 hover:bg-red-900/50 hover:border-red-700 transition-all flex items-center gap-1 cursor-pointer"><FaTrash size={9} /> Cancel</button>
                      )}
                    </div>
                  </div>
                </motion.div>
              ...
              {bookings.map((booking, i) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 rounded-full bg-black/70 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {booking.status !== 'cancelled' && (
                      <button onClick={() => handleCancel(booking._id)} className="p-2 bg-red-900 rounded-full text-red-400 hover:bg-red-800 cursor-pointer"><FaTrash size={10} /></button>
                    )}
                  </div>
                </motion.div>
              ))}
              ...
              {bookings.map((booking, i) => (
                ...
                >
                  ...
                  <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {booking.status !== 'cancelled' && (
                      <button onClick={() => handleCancel(booking._id)} className="p-1.5 bg-black/60 backdrop-blur rounded-lg text-red-400 hover:bg-black/80 cursor-pointer"><FaTrash size={9} /></button>
                    )}
                  </div>
                </motion.div>
              ))}
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <div className="flex gap-2">
                      ...
                      {booking.status !== 'cancelled' && (
                        <button onClick={() => handleCancel(booking._id)} className="px-4 py-2.5 bg-red-600/20 backdrop-blur border border-red-500/30 rounded-xl text-red-400 text-xs font-bold uppercase tracking-widest hover:bg-red-600/30 transition-all cursor-pointer"><FaTrash size={10} /></button>
                      )}
                    </div>
                  </div>
              ...
              {bookings.map((booking, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 px-4 py-3 border-t border-gray-800">
                    ...
                    {booking.status !== 'cancelled' && (
                      <button onClick={() => handleCancel(booking._id)} className="text-gray-400 hover:text-red-400 transition-colors cursor-pointer"><FaTrash size={14} /></button>
                    )}
                    <div className="flex-1" />
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-green-700/30">
                      ...
                      <div className="flex gap-1.5">
                        ...
                        {booking.status !== 'cancelled' && (
                          <button onClick={() => handleCancel(booking._id)} className="text-[9px] text-red-300 hover:text-red-200 font-bold uppercase tracking-wider cursor-pointer">Cancel</button>
                        )}
                      </div>
                    </div>
          ...
          >
            ...
            >
              <div className="text-center mb-5">
                ...
                <h3 className="text-lg font-black uppercase tracking-widest text-white" style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
                  View Modes
                </h3>
...
const BookingsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      {/* Tab Bar */}
      <div className="sticky top-0 z-30 px-4 pb-4 pt-2" style={{ background: 'rgba(3,7,18,0.92)', backdropFilter: 'blur(16px)' }}>
          ...
          {activeTab === 0 && (
            ...
            >
              ...
              >
                ...
                <p className="text-gray-500 text-sm">Select a court to view availability and book online</p>
              </motion.div>
              ...
              {courtsLoading ? (
                ...
                <div className="text-center py-20 text-gray-500">No courts available right now.</div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5 mb-8">
                  {courts.map((court, i) => (
              ...
              >
                <Link href="/">
                  ...
                  >
                    <FaFutbol size={13} /> View All Courts <FaArrowRight size={11} />
                  </motion.span>
                </Link>
              </motion.div>
            </motion.div>
          ...
          {activeTab === 1 && (
            ...
            >
              ...
              >
                ...
                <p className="text-gray-500 text-sm">Birthday parties, corporate events, tournaments — we do it all</p>
              </motion.div>
              ...
              >
                <Link href="/events">
                  ...
                  >
                    <FaCalendarAlt size={13} /> View Full Events Page <FaArrowRight size={11} />
                  </motion.span>
                </Link>
              </motion.div>
            </motion.div>
